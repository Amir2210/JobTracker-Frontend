...
export function Pagination({ totalJobs, filterBy, onSetFilter }: PaginationProps) {
  ...
  function renderPaginationButtons() {
    ...
    for (let i = 0; i < numOfButtons; i++) {
      buttons.push(
        <button key={i} onClick={() => handlePageChange(i)} className={`btn shadow-lg text-lg ml-4 ${filterBy.pageIdx === i ? 'bg-sky-400 text-white border-base-300 border' : 'bg-zinc-100 border-indigo-100 border'}`}>{i + 1}</button>
      )
    }
    return buttons
  }
